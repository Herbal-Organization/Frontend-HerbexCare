...
import {
  getAdminAiConsultations,
  deleteAdminAiConsultation,
...
function AdminAiConsultationsPage() {
  ...
  const load = async (options = {}) => {
    ...
    setError("");
    try {
      const response = await getAdminAiConsultations({
        PageNumber: nextPage,
        PageSize: nextSize,
      });
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const haystack = [item.patientName, item.consultationType, item.status]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-7xl">
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-200 md:text-base">
              View all AI consultations across the platform. Monitor usage and manage records.
            </p>
          </div>
          <button
            type="button"
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                  <thead className="bg-slate-50 dark:bg-slate-900">
                    <tr>
                      <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">ID</th>
                      <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Patient</th>
                      <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Type</th>
                      ...
                      <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Date</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
                    {filteredItems.map((item, index) => {
                      const id = item.id ?? item.consultationId ?? index;
                      return (
                        <tr key={id} className="transition-colors hover:bg-slate-50 dark:bg-slate-900/40">
                          ...
                          <td className="px-5 py-4">
                            <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                              {item.patientName || item.patientUserName || "N/A"}
                            </p>
                          </td>
                          <td className="px-5 py-4">
                            <p className="text-sm text-slate-700 dark:text-slate-300">
                              {item.consultationType || item.type || "—"}
                            </p>
                          </td>
                          <td className="px-5 py-4">
                            <span className={`inline-flex rounded-full px-3 py-1 text-xs font-black uppercase tracking-wide ${statusColor(item.status)}`}>
                              {item.status || "—"}
                            </span>
                          </td>
...
export default AdminAiConsultationsPage;
