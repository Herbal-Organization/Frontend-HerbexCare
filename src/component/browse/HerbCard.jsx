...
function HerbCard({ herb }) {
  ...
  return (
    <motion.button
      ...
      className="group flex flex-col overflow-hidden rounded-[2rem] border border-slate-100 bg-white text-left shadow-sm hover:shadow-xl transition-shadow duration-300 h-full w-full"
    >
      {herb.imageURL ? (
      ...
      <div className="p-6 flex-1 flex flex-col w-full">
        ...
        <div className="mt-auto pt-5 border-t border-slate-100 flex items-center justify-between">
          <p className="text-xs font-semibold text-slate-400">
            {herb.herbalistName
              ? `Curated by ${herb.herbalistName}`
              : "Curated by Herbalist"}
          </p>
          <div className="h-8 w-8 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors">
