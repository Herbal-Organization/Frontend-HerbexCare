...
import {
  getAdminInventoryHerbs,
  deleteAdminInventoryHerb,
} from "@api/inventory";
...
function AdminInventoryHerbsPage() {
  ...
  const load = async (options = {}) => {
    ...
    setError("");
    try {
      const response = await getAdminInventoryHerbs({
        PageNumber: nextPage,
        PageSize: nextSize,
      });
  ...
  const handleDelete = async (herbalistId, herbId) => {
    if (!window.confirm("Are you sure you want to delete this inventory record?")) return;
    const key = `${herbalistId}-${herbId}`;
    setDeletingId(key);
    try {
      await deleteAdminInventoryHerb(herbalistId, herbId);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const haystack = [item.herbName, item.herbalistName, item.scientificName]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-7xl">
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-200 md:text-base">
              View all herb inventory records across herbalists. Manage pricing and availability.
            </p>
          </div>
          <button
            type="button"
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                  <thead className="bg-slate-50 dark:bg-slate-900">
                    <tr>
                      <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Herb</th>
                      ...
                      <th className="px-5 py-3 text-left text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Price / kg</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
                    {filteredItems.map((item, index) => {
                      const key = `${item.herbalistId}-${item.herbId}-${index}`;
                        ...
                        <tr key={key} className="transition-colors hover:bg-slate-50 dark:bg-slate-900/40">
                          ...
                          <td className="px-5 py-4">
                            <p className="text-sm font-bold text-slate-900 dark:text-slate-100">
                              {item.pricePerKilo != null
                                ? `$${Number(item.pricePerKilo).toLocaleString()}`
                                : "—"}
                            </p>
                          </td>
                          <td className="px-5 py-4">
                          ...
                          <td className="px-5 py-4 text-right">
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDelete(item.herbalistId, item.herbId);
                              }}
...
export default AdminInventoryHerbsPage;
