...
import { getAdminFeedbacks, deleteAdminFeedback } from "@api/feedbacks";
...
function AdminFeedbacksPage() {
  ...
  const loadData = async ({ nextPage = pageNumber, nextSize = pageSize } = {}) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || "Unable to load feedbacks.";
      setError(msg);
      toast.error(msg);
    } finally {
      setIsLoading(false);
  ...
  const handleDelete = async (feedback) => {
    const id = String(feedback.feedbackId ?? feedback.id);
    ...
    return (
      <div className="flex gap-0.5">
        {Array.from({ length: 5 }, (_, i) => (
          <FaStar
            key={i}
            className={`text-xs ${i < count ? "text-amber-400" : "text-slate-200 dark:text-slate-600"}`}
          />
        ))}
      </div>
    );
    ...
    <div className="space-y-6 p-4 md:p-8">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-7xl">
            ...
            <h1 className="mt-4 text-3xl font-black tracking-tight md:text-5xl">
              {t("adminFeedbacks.title", "All Feedbacks")}
            </h1>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-200 md:text-base">
              {t("adminFeedbacks.subtitle", "View and moderate user feedbacks across all recipe types.")}
            </p>
          </div>
          <button type="button" onClick={() => loadData({ nextPage: pageNumber, nextSize: pageSize })} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-white/15">
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-100 dark:border-slate-700 pb-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Feedbacks list</h2>
        ...
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-4 py-12 text-center text-sm text-slate-500 dark:text-slate-400">No feedbacks found.</div>
        ) : (
          <div className="pt-5">
                    ...
                    {feedbacks.map((fb) => {
                      ...
                      return (
                        <tr key={id} className="transition-colors hover:bg-emerald-50/40 dark:hover:bg-emerald-900/20">
                          <td className="px-5 py-4 text-sm font-bold text-slate-900 dark:text-slate-100">{fb.userName || fb.patientName || "—"}</td>
                          <td className="px-5 py-4 text-sm text-slate-700 dark:text-slate-300">{fb.recipeName || fb.recipeTitle || "—"}</td>
                          <td className="px-5 py-4">{renderStars(fb.rating)}</td>
                          <td className="px-5 py-4 text-sm text-slate-500 dark:text-slate-400 max-w-xs truncate">{fb.comment || fb.review || "—"}</td>
                          <td className="px-5 py-4 text-right">
                            <button type="button" onClick={() => handleDelete(fb)} disabled={deleting} className="inline-flex items-center gap-1.5 rounded-xl border border-rose-200 dark:border-rose-800 px-3 py-2 text-xs font-bold text-rose-700 dark:text-rose-400 transition-colors hover:bg-rose-50 dark:hover:bg-rose-900/30 disabled:cursor-not-allowed disabled:opacity-60">
