...
import { getUserInitials, getUserRoleTone, formatUserLocation } from "@features/admin/services/adminUsers";
    ...
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Showing all system users returned by the live API.</p>
        </div>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{user.userName || user.email || "No username"}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${getUserRoleTone(user.role)}`}>
                    {user.role || "N/A"}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm font-medium text-slate-700">{user.email || "N/A"}</p>
                  <p className="text-xs text-slate-500">{user.phone || "No phone number"}</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm font-medium text-slate-700">{formatUserLocation(user)}</p>
                </td>
                <td className="px-6 py-4">
